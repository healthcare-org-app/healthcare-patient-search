"""HTTP routes for patient-search-service.

Generic CRUD over the `patient_search` table. Every mutating endpoint publishes a
...
def build_blueprint(svc) -> Blueprint:
    ...
    def search():
        ...
        rows = db.query(
            f"SELECT * FROM {TABLE} WHERE data::text ILIKE %s LIMIT 100",
            (f"%{q}%",),
        )
        emit_audit(bus, action=f"{RESOURCE}.search", actor=_actor(), details={"q": q})
        return jsonify({"count": len(rows), "items": [_row_json(r) for r in rows]})
